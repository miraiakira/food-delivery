...
const Verification: FC<Props> = ({ setActiveState }) => {
  ...
  const verificationHandler = async () => {
    ...
    if (verificationNumber.length !== 4) {
      ...
      try {
        ...
        setActiveState(ActiveState.Login);
      } catch (error: any) {
  ...
  const handleInputChange = (index: number, value: string) => {
    ...
    let newVerifyNumber = { ...verifyNumber };
    if (value.length > 1) {
      value.split('').forEach((num, idx) => {
        const newIdx = idx + Number(index);
        if (newIdx <= 3) {
          newVerifyNumber = { ...newVerifyNumber, [newIdx]: num };
        }
      });
    } else {
      newVerifyNumber = { ...verifyNumber, [index]: value };
    }
